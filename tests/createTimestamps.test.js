...
const defaultCues = [
  ...
  },
  {
    ...
    startMode: CueStartMode.LOCKED,
    duration: 30 * 60000, // 30 min
  },
]
const defaultCueOrder = [{ id: '#1' }, { id: '#2' }, { id: '#3' }, { id: '#4' }]
const defaultRunner = {
  ...getRunnerDefaults(),
  timesnap: {
    cueId: '#1',
  ...
  originalCues: {
    ...
    '#4': _.pick(defaultCues[3], ['startTime', 'duration']),
  },
  elapsedCues: {},
}
...
describe('createTimestamps', () => {
  ...
  describe('Not running, default & control cases, UTC', () => {
    it('Not running, runner is null', () => {
      ...
      expect(timestamps.ideal).to.deep.equal({ start: new Date('2024-07-26T09:00:00.000Z'), duration: 90 * 60000 })
      ...
      expect(timestamps.cues['#4']).to.deep.equal({
        ...
        ideal: { start: new Date('2024-07-26T10:00:00.000Z'), duration: 30 * 60000 },
